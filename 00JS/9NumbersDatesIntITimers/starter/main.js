...
const formatMovementDate = function (date, locale) {
  ...
  if (daysPassed <= 7) retur`${daysPassed} days ago`;
...
const displayMovements = function (acc, sort = false) {
  ...
  // creates an array of object e.i (mov and movdate sorted in sync together)
  //   [
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    // decrease 1s
    // when at 0, stop timer and log out user
    if (time === 0) {
      clearInterval(timer);
  ...
  // set countdown to 5minutes
  let time = 10;
  // call teh timer every second
